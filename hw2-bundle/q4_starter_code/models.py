...
class GraphSage(pyg_nn.MessagePassing):
    ...
    def __init__(self, in_channels, out_channels, reducer='mean', 
        ...
        self.lin = nn.Linear(in_channels, out_channels) # linear transformation to be applied for central node
        self.agg_lin = nn.Linear(in_channels, out_channels) # linear transformation to be applied for aggregated message from neighbors
    ...
    def update(self, aggr_out):
        ############################################################################
        # Your code here! Perform the update step here.
...
class GAT(pyg_nn.MessagePassing):
    ...
    def message(self, edge_index_i, x_i, x_j, size_i):
        ...
        ############################################################################
        #  TO-DO: Your code here! Compute the attention coefficients alpha as described
        # in equation (7). Remember to be careful of the number of heads with 
        # dimension!
        # Our implementation is ~5 lines, but don't worry if you deviate from this.

        x_i = x_i.view(-1, self.heads, self.out_channels) # the size -1 is inferred from other dimensions (batch, K, F')
        x_j = x_j.view(-1, self.heads, self.out_channels) # (batch, K, F')
        cat_x = torch.cat([x_i, x_j], dim=-1) # (batch, K, 2*F')
        alpha = F.leaky_relu((cat_x * self.att).sum(dim=-1), negative_slope=0.2) # leaky relu on dot product of shape (batch, K)
